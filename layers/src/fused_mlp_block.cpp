...
void fused_text_mlp_swiglu_m2(
    ...
    for (int kk = 0; kk < K2; kk += group_size) {
        ...
        for (int k = kk; k < kk + group_size; k += 8) {
            __m512 f0 = _mm512_loadu_ps(t_ptr + k);
    ...
    for (size_t jj = 0; jj < N; ++jj) {
        __m512 c0_up_f = _mm512_setzero_ps();
        __m512 c1_up_f = _mm512_setzero_ps();
        ...
        const int *__restrict up_sum_ptr = w_up_sum_int8 + (jjK_g << 4);
        
        for (size_t kk = 0; kk < K; kk += group_size) {
            const size_t g_off = kk / group_size;
            __m512i c0_up = _mm512_setzero_si512();
            __m512i c1_up = _mm512_setzero_si512();

            for (size_t k = kk; k < kk + group_size; k += 64) {
                __m512i b_up = _mm512_loadu_si512((__m512i*)(up_w_ptr + k));

                __m512i a0_vec = _mm512_loadu_si512((__m512i*)(a_q8 + k));
                __m512i a1_vec = _mm512_loadu_si512((__m512i*)(a1_ptr + k));
                
                c0_up = _mm512_dpbusd_epi32(c0_up, a0_vec, b_up);
                c1_up = _mm512_dpbusd_epi32(c1_up, a1_vec, b_up);
            }

            __m512i corr32 = _mm512_loadu_si512((__m512i*)(up_sum_ptr + (g_off << 4)));

            c0_up = _mm512_sub_epi32(c0_up, corr32);
            c1_up = _mm512_sub_epi32(c1_up, corr32);

            const float up_s = up_s_ptr[g_off];
            
            c0_up_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c0_up), _mm512_set1_ps(a_q8_s[g_off] * up_s), c0_up_f);
            c1_up_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c1_up), _mm512_set1_ps(a_q8_s[K_g + g_off] * up_s), c1_up_f);
        }

        up_ptr[jj] = _mm512_reduce_add_ps(c0_up_f);
        up_ptr[N + jj] = _mm512_reduce_add_ps(c1_up_f);
    }
    
    #pragma omp parallel for schedule(static)
    for (size_t jj = 0; jj < N; ++jj) {
        __m512 c0_gate_f = _mm512_setzero_ps();
        __m512 c1_gate_f = _mm512_setzero_ps();

        const size_t jjK = jj * K;
        const size_t jjK_g = jj * K_g;
        const int8_t *__restrict gate_w_ptr = w_gate_w + jjK;
        const float *__restrict gate_s_ptr = w_gate_s + jjK_g;
        const int *__restrict gate_sum_ptr = w_gate_sum_int8 + (jjK_g << 4);
        ...
        for (size_t kk = 0; kk < K; kk += group_size) {
            ...
            for (size_t k = kk; k < kk + group_size; k += 64) {
                __m512i b_up = _mm512_loadu_si512((__m512i*)(gate_w_ptr + k));
            ...
            __m512i corr32 = _mm512_loadu_si512((__m512i*)(gate_sum_ptr + (g_off << 4)));
            ...
            c0_gate_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c0_up), _mm512_set1_ps(a_q8_s[g_off] * gate_s), c0_gate_f);
            c1_gate_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c1_up), _mm512_set1_ps(a_q8_s[K_g + g_off] * gate_s), c1_gate_f);
        }
        
        const float gate0_tmp = _mm512_reduce_add_ps(c0_gate_f); 
        const float silu0 = gate0_tmp / (1.0f + expf(-gate0_tmp));  // SiLU(x) = x * sigmoid(x)
        gate_ptr[jj] = up_ptr[jj] * silu0;
        
        const float gate1_tmp = _mm512_reduce_add_ps(c1_gate_f); 
...
void fused_text_mlp_swiglu_m4(
    ...
    for (size_t jj = 0; jj < N; ++jj) {
        __m512 c0_up_f = _mm512_setzero_ps();
        __m512 c1_up_f = _mm512_setzero_ps();
        __m512 c2_up_f = _mm512_setzero_ps();
        __m512 c3_up_f = _mm512_setzero_ps();
        ...
        for (size_t kk = 0; kk < K; kk += group_size) {
            ...
            __m512i c0_up = _mm512_setzero_si512();
            __m512i c1_up = _mm512_setzero_si512();
            __m512i c2_up = _mm512_setzero_si512();
            __m512i c3_up = _mm512_setzero_si512();
            ...
            for (size_t k = kk; k < kk + group_size; k += 64) {
                ...
                c0_up = _mm512_dpbusd_epi32(c0_up, a0_vec, b_up);
                c1_up = _mm512_dpbusd_epi32(c1_up, a1_vec, b_up);
                c2_up = _mm512_dpbusd_epi32(c2_up, a2_vec, b_up);
                c3_up = _mm512_dpbusd_epi32(c3_up, a3_vec, b_up);
            }
            ...
            c0_up = _mm512_sub_epi32(c0_up, corr32);
            c1_up = _mm512_sub_epi32(c1_up, corr32);
            c2_up = _mm512_sub_epi32(c2_up, corr32);
            c3_up = _mm512_sub_epi32(c3_up, corr32);

            const float up_s = up_s_ptr[g_off];
            
            c0_up_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c0_up), _mm512_set1_ps(a_q8_s[g_off] * up_s), c0_up_f);
            c1_up_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c1_up), _mm512_set1_ps(a1_s_ptr[g_off] * up_s), c1_up_f);
            c2_up_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c2_up), _mm512_set1_ps(a2_s_ptr[g_off] * up_s), c2_up_f);
            c3_up_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c3_up), _mm512_set1_ps(a3_s_ptr[g_off] * up_s), c3_up_f);
        }

        up_ptr[jj] = _mm512_reduce_add_ps(c0_up_f);
        up_ptr[N + jj] = _mm512_reduce_add_ps(c1_up_f);
        up_ptr[(N << 1) + jj] = _mm512_reduce_add_ps(c2_up_f);
        up_ptr[(N * 3) + jj] = _mm512_reduce_add_ps(c3_up_f);
    }
    ...
    for (size_t jj = 0; jj < N; ++jj) {
        __m512 c0_gate_f = _mm512_setzero_ps();
        __m512 c1_gate_f = _mm512_setzero_ps();
        __m512 c2_gate_f = _mm512_setzero_ps();
        __m512 c3_gate_f = _mm512_setzero_ps();
        ...
        for (size_t kk = 0; kk < K; kk += group_size) {
            const size_t g_off = kk / group_size;
            __m512i c0_gate = _mm512_setzero_si512();
            __m512i c1_gate = _mm512_setzero_si512();
            __m512i c2_gate = _mm512_setzero_si512();
            __m512i c3_gate = _mm512_setzero_si512();

            for (size_t k = kk; k < kk + group_size; k += 64) {
                __m512i b_gate = _mm512_loadu_si512((__m512i*)(gate_w_ptr + k));
                ...
                c0_gate = _mm512_dpbusd_epi32(c0_gate, a0_vec, b_gate);
                c1_gate = _mm512_dpbusd_epi32(c1_gate, a1_vec, b_gate);
                c2_gate = _mm512_dpbusd_epi32(c2_gate, a2_vec, b_gate);
                c3_gate = _mm512_dpbusd_epi32(c3_gate, a3_vec, b_gate);
            }

            __m512i corr32 = _mm512_loadu_si512((__m512i*)(gate_sum_ptr + (g_off << 4)));

            c0_gate = _mm512_sub_epi32(c0_gate, corr32);
            c1_gate = _mm512_sub_epi32(c1_gate, corr32);
            c2_gate = _mm512_sub_epi32(c2_gate, corr32);
            c3_gate = _mm512_sub_epi32(c3_gate, corr32);

            const float gate_s = gate_s_ptr[g_off];
            
            c0_gate_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c0_gate), _mm512_set1_ps(a_q8_s[g_off] * gate_s), c0_gate_f);
            c1_gate_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c1_gate), _mm512_set1_ps(a1_s_ptr[g_off] * gate_s), c1_gate_f);
            c2_gate_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c2_gate), _mm512_set1_ps(a2_s_ptr[g_off] * gate_s), c2_gate_f);
            c3_gate_f = _mm512_fmadd_ps(_mm512_cvtepi32_ps(c3_gate), _mm512_set1_ps(a3_s_ptr[g_off] * gate_s), c3_gate_f);
        }
        ...
        const float gate2_tmp = _mm512_reduce_add_ps(c2_gate_f); 
        const float silu2 = gate2_tmp / (1.0f + expf(-gate2_tmp));  // SiLU(x) = x * sigmoid(x)
        gate_ptr[(N << 1) + jj] = up_ptr[(N << 1) + jj] * silu2;
        
        const float gate3_tmp = _mm512_reduce_add_ps(c3_gate_f); 
