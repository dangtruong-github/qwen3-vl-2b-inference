...
void gemv_bias_lg_N_K(
    ...
    for (size_t jj = 0; jj < N; ++jj) {
        ...
        mat_C[jj] = add_reduce_mm_256(c0_f) + mat_bias[jj];
    }
}

void gemv_bias_lg_N_K_g128(
    const float *__restrict mat_A,
    const int8_t *__restrict mat_B_in,
    const float *__restrict mat_B_scales,
    const float *__restrict mat_bias,
...
void f32a_i8f32sb_f32bias_f32c_avx2_kernel(
    ...
    if (M == 1 && N >= 1024 && K >= 1024) {
        if (K <= K_BLOCK) {
            if (group_size == 128) {
                gemv_bias_lg_N_K_g128(mat_A, mat_B_in, mat_B_scales, mat_bias, mat_C, N, K);
            } else {
                gemv_bias_lg_N_K(
                    mat_A, mat_B_in, mat_B_scales, mat_bias, mat_C, N, K, group_size
                );
            }
        } else {
            fprintf(stderr, "Decode error: M=%zu, N=%zu, K=%zu\n", M, N, K);
            exit(1);
        }
        return;
    }
    ...
    if (K <= K_BLOCK) {
        if (group_size == 128) {
            ...
            for (size_t i = 0; i < M; ++i) {
                gemv_bias_lg_N_K_g64(mat_A, mat_B_in, mat_B_scales, mat_bias, mat_C, N, K);
                mat_A += K;
                mat_C += N;
            }
